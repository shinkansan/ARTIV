...
class rosPub:
    ...
    def data_pub(self, data):
        if (len(data) < 18):
            print ("Vehicle Data is not ready... {%d}/18 set" %(len(data)))
            print("Wait...")
            return
        ...
        artmsg.avgSpeed = max([self.WheelRR, self.WheelFR, self.WheelRL, self.WheelFL])/10
        ...
        jointmsg.velocity = [artmsg.avgSpeed, self.WheelFL, self.WheelFR, self.WheelRL, self.WheelRR]
