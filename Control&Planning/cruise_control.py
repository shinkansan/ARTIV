...
def cruise(accelPub_, brakePub_):
    ...
    prev_time = 0.0
    desire_accel = 0
    K_p, K_d, K_i = 0.15, 1, 0.0003'''
    while(1):
        if cruise_mode == 1:
            #accel_ = Int16()
            #brake_ = Int16()
            '''cur_time = time.time()
            error = cruise_speed - velocity
            if error > 0:
                error_accel = desire_accel - cur_accel
                accel_out = K_p * error_accel + K_d * (error_accel - prev_accel)/(cur_time - prev_time) + K_i * intergral_accel
                prev_accel = error_accel
                integral += error_accel * (cur_time - prev_time)
                prev_time = cur_time
            
            PID_out = K_p * error + K_d * (error - prev)/(cur_time-prev_time) + K_i * integral
            prev = error
            integral += error * (cur_time - prev_time)
            prev_time = cur_time    '''
            error = cruise_speed - velocity
            if abs(error) < 2:
                accel_.data = 850
