...
def FeedbackWriter(fobject, testCase, desiredVal, val='accel'):
    global callbackData, Loginterval, duration, current
    while(True):
        
        if type(callbackData) == list:
            break
    start1 = current
    end = start1 + duration
    if val=='accel':
...
def test(duration, val='accel', accel_val=0, brake_val=0, steer_val = 0, gear_val = 0):
    ...
    gear.data = 0
    start = current
    max_time_end = start + duration
    if val == 'accel' or val == 'reset':  
        accelACT = int(accel_val)
        accel.data = accelACT
    elif val == 'brake'or val == 'reset':
    ...
    while True:
        ...
        elif val == 'steer' or val == 'reset':   
            steerPub.publish(steer)
        elif val == 'gear' or val == 'reset':
...
def doTest():
    ...
    for tc in range(testCase):
        ...
        FeedbackWriter(fileWrite, tc, accelTestValMax, val='accel')
        testThread.join()
    ...
    for tc in range(testCase):
        ...
        FeedbackWriter(fileWrite, tc, brakeTestValMax, val='brake')
        testThread.join()
